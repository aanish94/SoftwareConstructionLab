...
//******************NEW GLOBAL VARIABLES FOR MULTI-THREADING
int nthreads;
scene_t scene;
...
{
  long tID = (long)arg;
  int t1 = tID * width/nthreads;
  int t2 = (tID + 1) * width/nthreads;
      /* for every pixel */
    for( int px=t1; px<t2; ++px )
    {
	    ...
	    int count = 3 * py;
            ...
            scaled_color[px][count + 0] = gamma( pixel_color[0] ) * max_color;
            scaled_color[px][count + 1] = gamma( pixel_color[1] ) * max_color;
            scaled_color[px][count + 2] = gamma( pixel_color[2] ) * max_color;

            /* enforce caps, replace with real gamma */
            for( int i=0; i<3; i++)
	      {
  scaled_color[px][count + i] = max( min(scaled_color[px][count + i], 255), 0);
	      }
        }
    }
  pthread_exit(0);
